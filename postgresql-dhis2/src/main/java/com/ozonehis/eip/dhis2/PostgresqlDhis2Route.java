 ...
 *
 * <p>The @Component annotation enables Spring Boot component scanning to discover this route.
 *
 * <p>This route supports resume-from-checkpoint capability via SyncStateService, allowing it to
 * recover from crashes or restarts by continuing from the last successful sync.
 */
@Component
public class PostgresqlDhis2Route extends RouteBuilder {
  ...
  @Autowired private SyncStateService syncStateService;
  ...
  public void configure() throws Exception {
    ...
    String cronExpression =
        getContext().resolvePropertyPlaceholders("{{eip.postgresql.query.cron:0 */2 * * * ?}}");
    log.info("PostgreSQL query cron expression: {}", cronExpression);

    // Main route: iterate over configured reports, run each report's SQL, aggregate, transform,
    // send
    // Users duplicate a report per facility and set orgUnit in the mapping; SQL should already be
    // filtered per report
    from("quartz://postgresqlQuery?cron={{eip.postgresql.query.cron:0 */2 * * * ?}}")
        ...
        .process(
            exchange -> {
              Dhis2MappingConfig.ReportMapping report =
                  exchange.getIn().getBody(Dhis2MappingConfig.ReportMapping.class);
              exchange.setProperty("DHIS2_REPORT_MAPPING", report);

              // Determine report ID and name for sync tracking
              String reportId =
                  (report != null && report.getId() != null) ? report.getId() : "default";
              String reportName =
                  (report != null && report.getName() != null)
                      ? report.getName()
                      : "Default Report";
              ...
              java.sql.Timestamp lastSyncTimestamp =
                  lastSyncTime != null
              ...
              if (report != null && report.getParameters() != null) {
                ...
                log.debug(
                    "Merged {} report parameters into SQL params: {}",
                    report.getParameters().size(),
                    report.getParameters().keySet());
              }
              ...
              log.info(
                  "Running report {} - {} (last sync: {})", reportId, reportName, lastSyncTime);
        ...
        .split(body())
        .log(
            "Processing DataValueSet: dataSet=${body[dataSet]}, period=${body[period]}, orgUnit=${body[orgUnit]}, dataValues=${body[dataValues].size()}")
        ...
        .process(
            exchange -> {
              ...
              int recordCount =
                  body instanceof java.util.List ? ((java.util.List<?>) body).size() : 1;
    ...
    from("direct:dhis2-send")
        ...
        .process(
            exchange -> {
              ...
              String baseUrl =
                  exchange.getContext().resolvePropertyPlaceholders("{{eip.dhis2.base.url}}");
        ...
        .doTry()
        .toD(
            "dhis2:post/resource?path=dataValueSets"
    ...
    from("jms:queue:dhis2-dlq?connectionFactory=#bean:connectionFactory&concurrentConsumers=1&jmsMessageType=Text")
        ...
        .process(
            exchange -> {
              ...
              String errorMsg =
                  String.format(
                      "DLQ - Failed to process message after %d redeliveries. Body: %s",
                      redeliveryCount, body);
              log.error(errorMsg);
        ...
        .process(
            exchange -> {
              ...
              if (exception != null) {
                ...
                if (exceptionMsg != null) {
                  ...
                  if (exceptionMsg.contains("url=")) {
                    try {
                      String url = exceptionMsg.substring(exceptionMsg.indexOf("url=") + 4);
                      if (url.contains(",")) {
                    ...
                    if (cause.getClass().getName().contains("RemoteDhis2ClientException")) {
                      try {
                        ...
                        java.lang.reflect.Method getResponseMethod =
                            cause.getClass().getMethod("getResponse");
                        if (getResponseMethod != null) {
                          Object response = getResponseMethod.invoke(cause);
                          if (response != null) {
                            errorDetails
                                .append("DHIS2 Response Object: ")
                                .append(response.toString())
                                .append("\n");

                            // Try to get response body
                            try {
                              java.lang.reflect.Method getBodyMethod =
                                  response.getClass().getMethod("body");
                              if (getBodyMethod != null) {
                                Object body = getBodyMethod.invoke(response);
                                if (body != null) {
                                  errorDetails
                                      .append("DHIS2 Response Body: ")
              ...
              if (httpStatusCode == 409) {
                ...
                errorDetails.append(
                    "This may not be a critical error - the data is already present.\n");
