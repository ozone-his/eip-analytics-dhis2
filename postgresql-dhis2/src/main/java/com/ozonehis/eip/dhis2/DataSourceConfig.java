...
/**
 * Configuration class for PostgreSQL DataSource. Uses PostgreSQL's simple datasource to avoid
 * HikariCP version conflicts with EIP client runtime.
 */
public class DataSourceConfig {
  ...
  /**
   * Registers the DataSource with Camel context. Uses Camel's property resolver to read from Spring
   * Boot properties/environment variables.
   */
  public static void registerDataSource(CamelContext camelContext) {
    ...
    String jdbcUrl =
        camelContext.resolvePropertyPlaceholders(
            "{{eip.postgresql.jdbc.url:jdbc:postgresql://postgresql:5432/analytics}}");
    String username =
        camelContext.resolvePropertyPlaceholders("{{eip.postgresql.username:postgres}}");
    String password =
        camelContext.resolvePropertyPlaceholders("{{eip.postgresql.password:postgres}}");
