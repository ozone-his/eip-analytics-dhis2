...
/**
 * Aggregates individual data rows into DataValueSet objects grouped by dataSet, period, and
 * orgUnit.
 *
 * <p>This follows the pattern from OpenMRS DHIS2 Reporting Module where data is grouped before
 * sending to DHIS2 for efficiency.
 *
 * <p>Uses configuration file (dhis2-mappings.yml) to map columns to DHIS2 data elements, similar to
 * how OpenMRS module uses DataValueTemplate mappings.
 */
public class DataValueSetAggregator implements Processor {
  ...
  public void process(Exchange exchange) throws Exception {
    Dhis2MappingConfig.ReportMapping reportMapping =
        exchange.getProperty("DHIS2_REPORT_MAPPING", Dhis2MappingConfig.ReportMapping.class);
    ...
    if (reportMapping == null) {
      ...
      log.debug(
          "Aggregating using report mapping: {} - {}",
          reportMapping.getId(),
          reportMapping.getName());
    }
  ...
  private void aggregateUsingConfiguration(
    ...
    for (Map.Entry<String, List<Map<String, Object>>> entry : groupedData.entrySet()) {
      ...
      for (Dhis2MappingConfig.DataValueMapping mapping : mappings) {
        for (Map<String, Object> row : groupRows) {
          ...
          if (categoryOptionCombo != null && !categoryOptionCombo.trim().isEmpty()) {
            ...
            if (cocValue != null
                && !cocValue.toString().trim().isEmpty()
      ...
      if (!dataValues.isEmpty()) {
        ...
        log.debug(
            "Created DataValueSet: dataSet={}, period={}, orgUnit={}, dataValues={}",
            dataSet,
            period,
            orgUnit,
            dataValues.size());
  ...
  private void aggregateUsingDefaultMapping(List<Map<String, Object>> rows, Exchange exchange) {
    ...
    for (Map.Entry<String, List<Map<String, Object>>> entry : groupedData.entrySet()) {
      ...
      for (Map<String, Object> row : groupRows) {
        ...
        String categoryOptionCombo =
            getStringValue(
                row, "categoryOptionCombo", "categoryoptioncombo", "category_option_combo");
        ...
        if (categoryOptionCombo != null
            && !categoryOptionCombo.trim().isEmpty()
