...
/**
 * File-based sync state service that stores sync state in /eip-home/sync-state.json Replaces
 * database-based storage for better portability and self-contained operation.
 */
@Service
public class SyncStateService {
  ...
  private Instant getDefaultSyncStart() {
    ...
    String configuredValue =
        configuredDefaultSyncStart != null ? configuredDefaultSyncStart.trim() : "";
    try {
      defaultSyncStart = Instant.parse(configuredValue);
    } catch (Exception e) {
      defaultSyncStart = Instant.parse("2020-01-01T00:00:00Z");
      log.warn(
          "Invalid eip.sync.default-start value '{}'. Using fallback {}",
          configuredValue,
          defaultSyncStart);
    }
    return defaultSyncStart;
  ...
  public Map<String, Object> getLastSyncState(String reportId) {
    ...
    if (allStates.containsKey(reportId)) {
      ...
      log.info("Retrieved sync state for report {}: status={}", reportId, state.get("sync_status"));
      return state;
    }
  ...
  public Instant getLastSyncTimestamp(String reportId) {
    ...
    if (state.get("last_sync_timestamp") == null) {
      ...
      log.debug(
          "No previous sync timestamp for report {}. Using default start {}",
          reportId,
          defaultStart);
      return defaultStart;
    }
    ...
    try {
      ...
      Instant defaultStart = getDefaultSyncStart();
      log.warn(
          "Error parsing last sync timestamp for report {}: {}. Using default start {}",
          reportId,
          e.getMessage(),
          defaultStart);
      return defaultStart;
