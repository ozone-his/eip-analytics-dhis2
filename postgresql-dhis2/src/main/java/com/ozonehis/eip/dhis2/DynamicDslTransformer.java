...
/**
 * Processor that applies dynamic DSL transformations to data. The transformation DSL is loaded from
 * configuration and executed using Groovy.
 */
public class DynamicDslTransformer implements Processor {
  ...
  public void process(Exchange exchange) throws Exception {
    ...
    if (transformationDsl == null || transformationDsl.isEmpty()) {
      ...
      transformationDsl =
          exchange
              .getContext()
              .resolvePropertyPlaceholders("{{eip.transformation.dsl:{{transformation.dsl:}}}}");
    }
    ...
    try {
      Map<String, Object> transformedData =
          executeDslTransformation(dataMap, transformationDsl, exchange);
  ...
  private Map<String, Object> executeDslTransformation(
      Map<String, Object> inputData, String dslScript, Exchange exchange) {
  ...
  private Map<String, Object> applyDefaultTransformation(Map<String, Object> inputData) {
    ...
    // Default transformation assumes input has: dataElement, period, orgUnit, value,
    // categoryOptionCombo
    dhis2Data.put("dataSet", inputData.getOrDefault("dataSet", "default"));
    dhis2Data.put(
        "completeDate",
        inputData.getOrDefault("completeDate", java.time.LocalDate.now().toString()));
    ...
    if (categoryOptionCombo != null
        && !categoryOptionCombo.toString().trim().isEmpty()
